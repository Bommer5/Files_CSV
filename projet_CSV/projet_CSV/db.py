...
def creer_base_de_donnees():
    ...
    # Vérification et suppression de l'ancienne base de données si elle existe
    if os.path.exists(database_filename):
        print(f"Suppression de l'ancienne base de données : {database_filename}")
...
def ajouter_data(departement_id, produit_id, catégorie_id, prix, quantité, purchase_date,unit_id,location_id,supplier_id):
    conn = sqlite3.connect('CSV.db')
    curseur = conn.cursor()
    try:
        curseur.execute('''
            INSERT INTO data (departement_id, produit_id, catégorie_id, prix, quantité,purchase_date,unit_id,location_id,supplier_id) 
            VALUES (?, ?, ?,?,?,?,?,?,?)
        ''', (departement_id, produit_id, catégorie_id, prix, quantité,purchase_date,unit_id,location_id,supplier_id))
